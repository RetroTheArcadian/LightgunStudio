...
using LighgunStudio.Core.Win32;
using System.Collections.Generic;
using LighgunStudio.Core.RawInput;

namespace LighgunStudio.Core.RawInput
{
    public static class RawInputHelper
    {
        ...
        public static RawInputController[] GetRawInputDevices(RawInputDeviceType[] AccecptedDeviceTypes)
        {
            ...
            if (Win32API.GetRawInputDeviceList(nint.Zero, ref deviceCount, (uint)dwSize) == 0)
            {
                nint pRawInputDeviceList = Marshal.AllocHGlobal((int)(dwSize * deviceCount));
                Win32API.GetRawInputDeviceList(pRawInputDeviceList, ref deviceCount, (uint)dwSize);

                for (int i = 0; i < deviceCount; i++)
                {
                    // On Window 8 64bit when compiling against .Net > 3.5 using .ToInt32 you will generate an arithmetic overflow. Leave as it is for 32bit/64bit applications
                    RawInputDeviceList rid = (RawInputDeviceList)Marshal.PtrToStructure(new nint(pRawInputDeviceList.ToInt64() + dwSize * i), typeof(RawInputDeviceList));
